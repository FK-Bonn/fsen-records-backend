...
from app.database import User, DBHelper, Election
...
async def save_election_data(data: ElectionData, current_user: User = Depends(get_current_user())):
...
async def get_history(election_id: str):
    with DBHelper() as session:
