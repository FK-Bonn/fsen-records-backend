...
def test_get_single_file_unauthorized(mocked_base_dir):
    ...
    response = client.get('/api/v1/file/Informatik/AFSG-HHP-2022-02-01--2023-03-31.pdf')
...
def test_try_path_traversal(mocked_base_dir, mocked_hook):
    (mocked_base_dir() / 'Informatik').mkdir(parents=True)
    response = client.get('/api/v1/file/this-will-be-replaced/something',
                          headers=get_auth_header(client, ADMIN))
    assert response.status_code == 404
        ...
        'Informatik': [
            {
                ...
                'request_id': '',
                'references': None,
                'filename': f'AFSG-Prot-2023-10-01-{PDF_HASH}.pdf',
                'tags': ['HHP'],
                'sha256hash': PDF_HASH,
                'file_extension': 'pdf',
                'uploaded_by': ADMIN if user == ADMIN else None,
